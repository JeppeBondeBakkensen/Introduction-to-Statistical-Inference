...
```{r, echo = F}
set.seed(1020)
data <- 1000*rweibull(100, 1.5,1)+47
```

We have the following data on 100 auto insurance claims:

```{r, echo = F}
hist(data, main = '', xlab = 'Claim Cost', ylab = 'Count')
```
...
```{r, echo = F}
hist(data, main = '', xlab = 'Claim Cost', ylab = 'Proportion', freq = FALSE)
dgam <- function(x) dgamma(x, shape = par.old[1], rate = par.old[2])
curve(dgam, from = 0, to = 4000,col = 'blue', add = TRUE)
```
